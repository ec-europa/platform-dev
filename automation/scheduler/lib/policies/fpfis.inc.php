...
function fpfis_configure_apachesolr(&$subsite) {
	...
	$commands = array(
		...
		sprintf('sql-query "%s"', "REPLACE INTO apachesolr_index_bundles (env_id,entity_type,bundle) VALUES ('solr','node','page');"),
		sprintf('sql-query "%s"', "REPLACE INTO apachesolr_index_bundles (env_id,entity_type,bundle) VALUES ('solr','node','article');"),
	);
...
function fpfis_push_files_to_web_servers(&$subsite) {
	...
	// paths to be synced, along with a human comment
	$sync_path = array(
		array(
			'path' => $subsite->master()->path('sites') . '/sites.list.php',
			'comment' => sprintf('Update sites.list.php for the "%s" subsite', $subsite->name()), 
			'push_args' => array('nas-reference', 'master'),
		),
		array(
			'path' => $subsite->master()->path('sites') . '/' . $subsite->name(),
			'comment' => sprintf('Add subdirectory for the "%s" subsite', $subsite->name()),
			'push_args' => array('nas-reference', 'master'),
		),
		array(
			'path' => $subsite->master()->path('rewritemap'),
			'comment' => sprintf('Update RewriteMap for the "%s" subsite', $subsite->name()),
			'push_args' => array('nas-reference', 'master'),
		),
	);
	
	// depending on the target cluster, we actually have two ways of pushing
	// files to our web servers.
	$sync_command = $subsite->master()->cluster()->syncCommand();
	if ($sync_command != 'git') {
		// the former method simply applies commands generated by the API
		foreach ($sync_path as $path) {
			$sync_command = $subsite->master()->cluster()->syncCommand($path['path']);
		...
		// set the GIT_FPFIS_AUTHOR e.v. so the Git hooks do not ask for our name interactively
		putenv('GIT_FPFIS_AUTHOR=FPFIS scheduler');

		// save the current working directory
		$saved_cwd = getcwd();

		// "git" means we are supposed to commit our changes and push them
		foreach ($sync_path as $path) {

			// chdir to the parent repository (to avoid "Not a git repository" errors)
			if (!chdir(dirname($path['path']))) {
				$reports[] = sprintf('unable to change directory to %s', $path['path']);
				continue;
			}

			// git add -- yes, one can pass an absolute path; Git ensures it is
			// within the working tree and issues a "<file> is outside
			// repository" error if needed
			$git_command = sprintf('git add %s', escapeshellarg($path['path']));
			...
			// git commit -- again, one can pass an absolute path; this helps
			// ensuring we commit only the target file, whatever was staged
			// before.
			$git_command = sprintf('git commit --message=%s %s', escapeshellarg($path['comment']), escapeshellarg($path['path']));
			$git_exec = execute_command($git_command);
			if ($git_exec['code'] !== 0) {
				$reports[] = sprintf('%s failed with status code %d: %s', $git_command, $git_exec['code'], $git_exec['output']);
				continue;
			...
			// git push
			// note: it is less optimized but more perennial to issue a git push
			// after each commit; indeed, we have no guarantee that all the files we
			// have to handle are in the same git repository.
