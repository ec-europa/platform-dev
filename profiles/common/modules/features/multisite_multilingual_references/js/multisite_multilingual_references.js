/**
 * @file
 * Multilingual references javascript file.
 */
...
(function ($) {
    Drupal.behaviors.multilingual_entity_references = {
        attach: function (context, settings) {
            ...
            $("span." + locate_class).addClass(process_class).mouseleave(function () {
                $(this).hide();
            }).children().filter("span").each(function () {
                // Calculate width of the first "max_items_row" children.
                var w = 1;
                // 1px borders.
                $(this).children().filter("a:lt(" + max_items_row + ")").each(function () {
