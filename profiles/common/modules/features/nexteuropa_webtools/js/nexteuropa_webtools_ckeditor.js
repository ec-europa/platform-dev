...
(function ($) {
     ...
     */
    Drupal.behaviors.nexteuropa_block_ckeditor = {
        attach: function (context) {
            Drupal.nexteuropa_block_ckeditor = Drupal.nexteuropa_block_ckeditor || {};
            var editor_id = Drupal.nexteuropa_block_ckeditor.current_editor_id || '';

          if (editor_id in Drupal.nexteuropa_block_ckeditor) {
              var editor = Drupal.nexteuropa_block_ckeditor[editor_id];
              var content = context[0];

              $(content).find('.token-ckeditor-tips-toggle').once('token-ckeditor-tips-toggle', function() {
                  $(this).click(function (e) {
                      e.preventDefault();
                      $('.webtools-ckeditor-tips-container').toggle();
                  });
              });
                ...
                $(content).find('.token-ckeditor-selection').once('token-ckeditor-selection', function() {
                    $(this).click(function (e) {
                      ...
                      if (type != 'url') {
                        ...
                        $(content).find('.cke_dialog_close_button').click();
                    });
                });
          }
        }
