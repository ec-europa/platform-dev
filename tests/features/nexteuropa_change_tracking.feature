Feature: Change tracking features
  In order to easy communication between editors while they manage website
  content, a tracking feature is available with WYSIWYG fields.
  I want to use it while creating, editing content and I want to see tracked
  changes while viewing content draft.
  ...
  @api
  Scenario Outline: If WYSIWYG workflow settings are correctly configured, The
  change of the content state to "validated" or "published" must be blocked if
