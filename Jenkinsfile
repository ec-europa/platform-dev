    ...
    try {
        stage('Build & Install ' + label) {
            ...
            withCredentials([
                ...
                sh "./bin/phing build-platform-dev -Dcomposer.bin=`which composer` -D'drush.verbose'='true' -D'behat.base_url'='$BASE_URL/$SITE_PATH/build' -D'behat.wd_host.url'='$WD_HOST_URL' -D'behat.browser.name'='$WD_BROWSER_NAME' -D'behat.screenshots.path'='${BEHAT_SCREENSHOTS_PATH}' -D'env.FLICKR_KEY'='$FLICKR_KEY' -D'env.FLICKR_SECRET'='$FLICKR_SECRET' -D'integration.server.port'='$HTTP_MOCK_PORT' -D'varnish.server.port'='$HTTP_MOCK_PORT' -D'platform.profile.name'='$PLATFORM_PROFILE' -D'platform.site.theme_default'='$THEME_DEFAULT'"
